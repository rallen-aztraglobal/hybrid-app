...
enum class ProbeError {
    /**
     * 站点在服务（HTTP 2xx/3xx，或活着但拒绝本次请求的 401/403/451）且证书覆盖目标域名 —— 唯一的命中。
     * 证书=反劫持的密码学保证；403 等 geo/WAF 拦截视作命中，见 [DomainProber.probe] 说明。
     */
    HIT,
    ...
    /** 其它（404/410 等「不提供内容」的 4xx、解析异常等）—— 该域名当前不可服务，试下一个。
     *  注：401/403/451（geo/WAF 拦截，站点仍活着）不归此类，走 [HIT]/[HIJACK] 证书校验。 */
    OTHER,
}
