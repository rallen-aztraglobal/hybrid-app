...
///
/// 注意本包 A 面是**深色**的，而 B 面是浅色品牌站，所以切过去时底色会明显变亮。
/// 这与 decktallypro（深色游戏 → 浅色站）的情况相同，是既有口径。
/// 留白之所以仍用白色而不是跟 A 面同色：留白要和**站点**连成一体，
/// 用深色会在浅色站四周镶一圈黑边，比切换时亮一下更难看。
...
extension WebContainerViewController: WKNavigationDelegate {
    ...
    // 加载失败也必须收掉转圈：只在 didFinish 里停的话，断网或域名被封时会永远转圈
    // 盖在白屏上，用户既看不到内容也看不出「加载失败」，只能杀进程。
