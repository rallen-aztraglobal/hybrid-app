...
> `Core/Services/RecordStore.swift`、`Core/Services/SettingsStore.swift`），
...
| SDK / 请求 | 是否打进包 | **当前构建里是否真的上报** |
| --- | --- | --- |
| AppsFlyer | `project.pbxproj` 已声明 SPM 包，代码用 `#if canImport(AppsFlyerLib)` 守卫 | **否** —— `GateConfig.appsFlyerAppleAppID` 仍是 `TODO_APPSTORE_APP_ID`，`TrackingService.startAppsFlyer` 里的 `GateConfig.isConfigured(...)` 把它拦在 `initialize` 之前 |
| Adjust | 同上（`AdjustSdk`） | **否** —— `GateConfig.adjustAppToken` 仍是 `TODO_ADJUST_APP_TOKEN`，`startAdjust` 同样被 `isConfigured(...)` 拦下 |
| Firebase Cloud Messaging | 同上（`FirebaseMessaging`） | **看情况** —— `GoogleService-Info.plist` 尚未放入，缺文件时 `PushService.start` 在第一行 `guard Bundle.main.path(forResource:...)` 处整段返回；plist 一放进来就会取 token 并上报 |
| 启动配置请求 | 无 SDK，系统 `URLSession` | **是** —— `GateService.evaluate` 每次启动都会发 |

**两个归因 SDK 当前一个都不会初始化**，这是 `GateConfig` 里两个占位符共同决定的，
不是配置疏漏 —— 要等 App Store Connect 与 Adjust 后台建好条目才拿得到真值
（见 `README_GATE.md` §2 与 `RELEASE_SIGNING.md` 第 11 节）。

**尽管如此，申报按「上线后会启用」的口径写**，即 AppsFlyer / Adjust / FCM 三者都按会收集申报。
理由与其余上架包一致：SDK 已经打进包里，填上 key 就开始上报；
**申报宽于实现是安全方向，反过来（实现宽于申报）才是下架风险。**

一个例外见文末「ATT 与 Data Used to Track You」—— 那一项如果走「暂不启用归因」路线
（也就是**维持当前状态**上线），必须把申报一并收窄，不能只改一半。
...
> 不能答 No：即便游戏成绩完全不出设备，启动时的配置请求、FCM token 注册、
...
> 勾了 Data Used to Track You 之后，Apple 会在商品页上显示 "Data Used to Track You"
> 一栏，并且**要求 App 实现 ATT 授权请求**。本包目前**没有实现 ATT 请求**，
> 见文末，提交前必须处理。
...
> 这一栏空着是本包的实际情况，不是偷懒。如果以后加了账号体系、云端存档、
> 或者把玩家填的任何个人信息随请求发出去，就必须重新分栏。
...
> Health & Fitness · Financial Info · Payment Info · Purchases · Credit Info ·
> Location（见下）· Sensitive Info · Contacts · Browsing History · Search History ·
> User Content（Photos, Videos, Audio, **Gameplay Content**, Customer Support,
> Emails or Text Messages, Other User Content）· Diagnostics → Crash Data /
> Performance Data。

---

## 待定项：Gameplay Content —— **不申报**

**不勾。** 这是 Games 类目下最容易被填错的一项，把理由写死在这里：

Apple 的 **User Content → Gameplay Content** 指「用户在游戏中产生并**被收集上来**的内容」
（存档、关卡、对战录像等）。本 App 产生的「内容」只有各棋盘尺寸的**最少步数、最短用时、
通关次数**三个数字，它们：

- 存在 `UserDefaults`（`Core/Services/RecordStore.swift`，键 `gsl.records`，
  值是一小段 JSON），加上 `SettingsStore` 的 `gsl.boardSize` 与 `gsl.hapticsEnabled`；
- **从不上传、不同步、不进任何请求体、不作为归因事件参数**；
- 没有排行榜、没有分享入口、他人完全看不到；
- 卸载 App 即随沙盒数据一并删除，设置页的 "Reset all records" 也能当场清空。
...
>
> ⚠️ **红线**：一旦有任何代码把成绩发到服务器或第三方（云存档、排行榜、
> 把 `bestMoves` 当作归因事件参数……），这一项就必须改成
> **User Content → Gameplay Content**，且很可能变成 **Data Linked to You**。
> 改 `RecordStore` 或新增任何上传路径时，回来重看这一段。

---

## 待定项：Location（Coarse Location）—— **必须在提交前定死**

App **不申请任何定位权限**（`GridSlide/Info.plist` 里只有三个键，
没有 `NSLocationWhenInUseUsageDescription` 之类，代码里也没有 CoreLocation），读不到 GPS。
...
| 是否提供删除数据的途径？ | **Yes** | 隐私政策里的联系邮箱可请求删除；本地成绩用设置页的 "Reset all records" 或卸载即可清除 |
...
**现状**：`GridSlide/Info.plist` 里有 `NSUserTrackingUsageDescription`
（文案："GridSlide uses this to measure how well our app promotions work.
It never affects how you play."），但全工程没有 `ATTrackingManager` /
`AppTrackingTransparency`（`Gate/GateCoordinator.swift` 只调了
`TrackingService.shared.start()`）。也就是说 **App 目前不会弹 ATT 授权框**。

叠加另一件事：**两个归因 SDK 当前也都没启用**（key 是占位符）。
所以当前构建本身是自洽的 —— 没启用归因、拿不到也用不上 IDFA。
**不自洽的是「按上线后口径申报」与「当前实现」之间的差距**，这正是要拍板的地方。

`APP_STORE_LISTING.md` 的「IDFA / ATT」一节给了两条路线，**本文的申报必须跟着走同一条**：

| | 路线 A（补上 ATT + 填 key） | 路线 B（维持现状，暂不启用归因） |
| --- | --- | --- |
| 代码 | 加 `ATTrackingManager.requestTrackingAuthorization`，并填上 `appsFlyerAppleAppID` / `adjustAppToken` | 保持不变，`TODO_*` 占位符原样留着 |
| Data Used to Track You | **Yes**（按上文那三行勾） | **No**（整栏清空） |
| Data Not Linked to You | 按上表全填 | 去掉 AppsFlyer / Adjust 带来的 Advertising Data 与相应的 Analytics 用途；**FCM token 与配置请求的部分保留**（这两条与归因无关，照样在发） |
...
>
> 补 ATT 时注意**不要在冷启动第一屏弹**（授权率极低），等主界面呈现之后再请求。
