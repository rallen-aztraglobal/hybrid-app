...
class GateConfig {
  ...
  static const String bundleId = 'com.bellcroft.edgeloop8451';
  ...
  // —— AppsFlyer ——
  /// 账号级 devKey，与其余上架包同一个 AF 账号（AF 的 devKey 按账号发，不按 App 发）。
  static String get appsFlyerDevKey => 'fXoKsKQwxPCRdhD8CD8q6F';

  /// AF appId：Android 为包名（恒有效）；iOS 为 App Store 数字 id。
  /// 本包仅发 Android，iOS 侧留占位 —— AF 会因此在 iOS 保持 no-op。
  ...
  /// 其他包的 token —— 复用会把本包的安装与会话归到别的 App 上。
  ///
  /// **已填**（2026-09-11 建）。Adjust 里 App 名 `EdgeLoop`，Android 平台已配
  /// `com.bellcroft.edgeloop8451`（store Google Play），reporting currency PHP、
  /// 勾了「用户群仅在欧洲经济区之外」、App Links 关闭 —— 逐项与 ColorStack 一致。
  ///
  /// 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op。
  static String get adjustAppToken => 'g1usefapvwg0';
  ...
  /// `Color.WHITE`。
  ///
  /// **本包按浅色站取值**（品牌归属未定，这只是个与主题一致的预置值）。
  ///
  /// **当前这个值用不上，也不阻塞上架。** 本批包在 Console 里一律配**外开**：
  /// B 面交给系统浏览器，`web_screen.dart` 根本不会被走到。
  /// 所以定品牌时**不需要**为了这个常量重新打包 —— 它纯粹是 Console 里的一个字段。
  ///
  /// 那为什么还要让它跟品牌走？因为打开方式是**服务端下发**的：运营在 Console 里
  /// 把外开改成内开，客户端不重新打包就立刻生效，这个常量在那一刻才开始起作用。
  /// 真到那时，值不对只是 B 面边框颜色与站点主题不搭，不是故障 —— 修它要重新打包。
  ...
  ///
  /// **已填**（取自 `EdgeLoop` 的 Events 页）。`EdgeLoop` 下已建齐与 ColorStack 相同的
  /// 7 个事件；上架包本体只发 `OpenBLanding`，建齐只是为了保持同构。
  ///
  /// 与上面的 `adjustAppToken` 成对生效：`TrackingService.onOpenBLanding()`
  /// 要求两者都已配置才会发 Adjust 事件，缺一个就整条 no-op。
  static String get adjustOpenBLandingToken => 'tmzqo0';
}
