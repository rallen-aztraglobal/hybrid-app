...
本包接的是 ADR-0014 的上架包网关。**游戏本体对它零感知** ——
...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.bellcroft.edgeloop8451`，
palCode 填 `1053259233855094784`、品牌选 `ap`。

**本包挂 `ap`**（ArenaPlus 浅色站），PAL_CODE `1053259233855094784`（取自渠道 `ap01020`）。
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.bellcroft.edgeloop8451` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`g1usefapvwg0`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `tmzqo0`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFFFFFFFF`（挂 `ap`）。当前外开，这个值用不上，见上 |

Adjust 后台已给本包**单独建了 App**（名 `EdgeLoop`，Android 平台 `com.bellcroft.edgeloop8451`，
...
`EdgeLoop` 下已建齐与 ColorStack 相同的 7 个事件：`AddToCart` / `CompleteRegistration` /
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.bellcroft.edgeloop8451` 注册
（`mobilesdk_app_id` = `1:609439342540:android:ebba9a039b38a934a98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
该文件随 APK 分发、反编译即可得，非机密，进 git。

**已按规矩裁剪成只含本包一个 client。** 控制台下载的原始文件会把该项目下**所有**
Android App 都列进来（本次下载里有 11 条），理由见 `SECURITY_NOTES.md`。
...
adb shell am force-stop com.bellcroft.edgeloop8451
