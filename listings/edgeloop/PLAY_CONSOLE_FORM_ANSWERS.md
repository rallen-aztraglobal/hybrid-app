...
`https://precious-douhua-53c7dc.netlify.app/`

本包**专用**的 Netlify 站点，不与其他上架包共用。内容是 `store/privacy-policy.html`
改名 `index.html` 部署的，所以根路径直接就是政策页。

2026-09-14 实测（未登录）：HTTP 200、标题 `Privacy Policy — EdgeLoop`、
正文含本包包名与联系邮箱、无残留占位符。
...
本批游戏包共用这一个地址（本包 + NonoPix / LinkFlow / MineGrid）。
...
- [x] 33 项测试全过，analyze 干净
- [x] Adjust App 已建（`g1usefapvwg0`），`adjustAppToken` 与 `adjustOpenBLandingToken`（`tmzqo0`）已回填；7 个事件齐备
- [x] Firebase 已注册（`1:609439342540:android:ebba9a039b38a934a98769`，不加 SHA 指纹），裁剪过的 `google-services.json` 已放入
- [x] 隐私政策 URL 已上线且可匿名访问 —— `https://precious-douhua-53c7dc.netlify.app/`，实测 HTTP 200
...
- [x] 品牌归属已定：`ap`（ArenaPlus），PAL_CODE `1053259233855094784` —— Console 字段，未改任何常量、未重新打包
