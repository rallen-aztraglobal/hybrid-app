...
跑不通就先修代码，别急着弄证书。手验路径见 `README_GATE.md` 末节。
...
> 法定名称。本包若与 `decktallypro` 用**同一个** Apple Developer 账号，
> 两个 App 的 Seller 会**完全一致** —— 任何人点开商品页都能看出这两个 App 同属一家。
> 这与「各包使用互不相关的厂商命名空间（`deck` / `stillwater` / …）」的初衷直接冲突。
>
> 两个选项：
>
> | | 复用现有账号 | 另开一个开发者账号 |
...
| --- | --- |
| Type | App IDs → App |
| Description | PocketLedger |
| Bundle ID | **Explicit** → `com.stillwater.pocketledger` |
| Capabilities | 勾 **Push Notifications** |

> - Bundle ID 必须是 **Explicit**，不能用通配符 —— 推送与 App Store 上架都要求。
> - 只勾 Push Notifications。本 App 不用 iCloud、不用 App Groups、不用 Sign in with Apple，
>   多勾的 capability 会让描述文件与 entitlements 对不上，反而出签名错误。
...
> 手改 pbxproj 容易漏掉 Debug/Release 其中一份配置。
>
> 为什么切 Manual：Automatic 会让 Xcode 在归档时自作主张地创建 / 更新描述文件，
> 多包共用一个账号时很容易互相踩。与 `decktallypro` 保持同一做法。

### 5.2 勾 capability

Xcode → target PocketLedger → **Signing & Capabilities** → ＋ Capability：
...
`PocketLedger/PocketLedger.entitlements` 现在是：
...
| --- | --- |
| 平台 | **iOS**（只勾这一个） |
| 名称 | `PocketLedger`（见 `APP_STORE_LISTING.md`） |
| 主要语言 | English (U.S.) |
| 套装 ID | 选第 2 步建的 `com.stillwater.pocketledger` |
| SKU | 内部标识，随便取一个不重复的，例如 `pocketledger-ios-001`；不公开 |
| 用户访问权限 | 完全访问 |
...
| 内容 | 文档 |
| --- | --- |
| 名称 / 副标题 / 宣传文本 / 描述 / 关键词 / 类目 / URL | `APP_STORE_LISTING.md` |
| 年龄分级问卷 | `APP_STORE_LISTING.md` 的「年龄分级」一节 |
| App Privacy（隐私营养标签） | `APP_PRIVACY.md` |
| App Review Information（备注） | `APP_REVIEW_NOTES.md` |
...
3. 在同一个 Firebase 项目下以 bundleId `com.stillwater.pocketledger` 注册一个 iOS App，
   下载 `GoogleService-Info.plist` 放到 `PocketLedger/` 下
...
xcodebuild -scheme PocketLedger -project PocketLedger.xcodeproj \
  ...
  -archivePath build/PocketLedger.xcarchive archive

# 导出 .ipa（需要一份 ExportOptions.plist，见下）
xcodebuild -exportArchive \
  -archivePath build/PocketLedger.xcarchive \
...
<dict>
  ...
  <dict>
    <key>com.stillwater.pocketledger</key><string>AppStore</string>
  </dict>
...
codesign -d --entitlements :- unpacked/Payload/PocketLedger.app 2>/dev/null | grep -A1 aps-environment

# 2. 签名身份必须是 Apple Distribution
codesign -dvvv unpacked/Payload/PocketLedger.app 2>&1 | grep Authority

# 3. 图标必须无 alpha 通道
sips -g hasAlpha unpacked/Payload/PocketLedger.app/AppIcon*.png
```
...
- 命令行：
  ```bash
  xcrun altool --upload-app -f build/export/PocketLedger.ipa -t ios \
...
| 配置 | 值从哪来 |
| --- | --- |
| `GateConfig.appsFlyerAppleAppID` | 第 6 步记下的 App Store Connect **Apple ID**（纯数字） |
| `GateConfig.adjustAppToken` | Adjust 后台为本包**新建**的 App 的 12 位 App Token |

> Adjust 建 App 时：Platform 选 iOS、Store 选 App Store、
> bundleId 填 `com.stillwater.pocketledger`、reporting currency 与现有 app 对齐用 **PHP**
> （**建后不可改**）。
>
