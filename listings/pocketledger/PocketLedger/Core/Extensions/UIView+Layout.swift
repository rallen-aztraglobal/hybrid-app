...
final class MessageBannerView: UIView {
    ...
    init(message: String, style: BannerStyle) {
        ...
        backgroundColor = {
            switch style {
            case .info: return AppTheme.textPrimary.withAlphaComponent(0.92)
            case .success: return AppTheme.income.withAlphaComponent(0.95)
            case .failure: return AppTheme.expense.withAlphaComponent(0.95)
            }
        }()
