...
/// 启动闸编排：决定 App 展示 A 面（记账本体 MainTabBarController）还是 B 面（服务端下发 web）。
///
/// 流程：先把 window 根设为加载页并显示 → 启动归因 SDK（A/B 都 init）→ 异步判定 →
/// 据结果把根切成 MainTabBarController（A）或 WebContainerViewController（B）。
/// 判定失败/非 B 一律 A 面，记账本体代码零改动。
            ...
            await MainActor.run {
                ...
                case .bSide(let url, let openMode):
                    ...
                    case .externalBrowser:
                        // 外开：唤起系统浏览器打开 B 面，App 本体展示 A 面（记账）——
                        // 既送用户去 B 面，又让 App 看起来仍是干净的记账工具。
