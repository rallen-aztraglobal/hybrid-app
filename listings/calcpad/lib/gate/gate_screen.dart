...
/// 还是 B 面（服务端下发的 web）。判定完成前显示与计算器同底色的加载页，避免白屏。
///
/// 安全默认：判定进行中、失败、或结果非 B，一律进 A 面 —— 即计算器本体
/// (`CalculatorScreen`)，计算器侧代码零改动。
...
/// 判定期加载页。底色沿用计算器的 AppColors.background，判定结束切到计算器时背景不跳变。
      ...
      body: DecoratedBox(
        ...
        child: Center(
          child: SizedBox(
            width: 40,
            height: 40,
            // 用运算键的橙色，与计算器本体的视觉语言一致。
            child: CircularProgressIndicator(color: AppColors.operatorButton),
          ),
        ),
      ),
    );
