...
| 类别 | 文件 | 说明 |
| --- | --- | --- |
| 签名密钥 | `android/dayspan3306.jks` | release 签名密钥，泄露等于别人能签出被 Android 认作本 App 正式更新的包 |
...
```bash
git status --porcelain -uall listings/dayspan \
...
# 应为 0；把本包包名 com.haymoor.dayspan3306 代进去应 > 0
```

## google-services.json 的裁剪

放入前必须**只保留本包的 client**。Firebase 控制台下载的那份会把同一项目下
**所有** App 都列进来 —— 带着它出包，等于在 DaySpan 的 APK 里明写另外十几个上架包的
包名，正是各包使用互不相关的厂商命名空间（`haymoor` 等）想避免的事。
...
# 只应出现 com.haymoor.dayspan3306 一行
```
...
```bash
adb shell dumpsys package com.haymoor.dayspan3306 \
