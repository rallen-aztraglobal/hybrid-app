...
| `store/screenshot-1.png` | 1200×2400 | 手机截图 —— 天数差页 |
| `store/screenshot-2.png` | 1200×2400 | 手机截图 —— 工作日页 |
...
1. **截图必须补边到 1200 宽。** 实机是 1080×2400（9:20），比 Play 允许的最长边比例
   **2:1** 还要瘦，直接传会被拒。左右各补 60px 的 `#F6F5F2`（与 App 背景同色，
   看不出是补的）。**不要改成裁剪。**
...
- **图标**：3×3 的日子格，连续一段被数出来。

> 前两版都被推翻过：v1「两个大方块夹一根横杠」活像一只哑铃，
> 而这批里真正的健身 App 是 RepTimer；v2「一排五个竖条」像条形码。
- **特色图**：7 列 × 3 行的日子格，其中连续一段被数出来（起止两天深色、数进去的主色、其余浅灰）。

> 行数取 3 而不是一个月的 5，是被画布比例逼的：1024×500 是 2.05:1，
> 而 7×5 的网格只有 1.4:1，按高度撑满后宽度只占到画布的 40%，整块图缩在中间。
...
1. 两个日期、含首尾开关、结果，以及周/工作日/周末三格统计
2. 同一区间的工作日数与口径说明
...
| 项 | 状态 |
| --- | --- |
| 支持邮箱 | ✅ `toolsa069@gmail.com` —— **与 UnitShift / TickPad / CheckLane / RepTimer 共用**（运营决定） |
| 隐私政策 URL | ✅ `https://stellular-gumption-304771.netlify.app/` —— 本包专用 Netlify 站点 |
