...
> can vary by region. The tools and the user interface are identical for all
...
本包与 EdgeLoop / RepTimer 是同一批，与更早的 nonopix / linkflow / minegrid / unitshift /
...
| 项目 | 要求 | 本包 |
| --- | --- | --- |
| 包名 | 各取不相关的厂商命名空间 | `haymoor` |
| 证书 `O=` | **每包另取** | 见 `RELEASE_SIGNING.md` |
| 隐私政策 URL | **每包另建站点** | ✅ `https://stellular-gumption-304771.netlify.app/` |
| 支持邮箱 | ⚠️ **按游戏/工具分两组共用** | `toolsa069@gmail.com` |
| `google-services.json` | 必须裁剪成只留本包 client | 见 `SECURITY_NOTES.md` |
| Adjust App Token | 必须新建 App，不可复用 | ✅ `jmnqnzl3ury8` |
...
本包的支持邮箱是 `toolsa069@gmail.com`，与 UnitShift / TickPad / CheckLane / RepTimer **共用一个**。
...
| 代码与测试 | ✅ analyze 干净，36 项测试全过；release APK 53.4MB |
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `jmnqnzl3ury8` / `3e00vd` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.haymoor.dayspan3306` 一条 client |
| 支持邮箱 | ✅ `toolsa069@gmail.com`（**注意与其余工具包共用**）|
| 隐私政策 URL | ✅ `https://stellular-gumption-304771.netlify.app/`（实测匿名 HTTP 200）|
| 品牌归属 | ✅ `bp`（BingoPlus），打开方式**外开**，`bSideChromeColor` = `0xFFFFFFFF`（主题吻合，未改常量）|
...
2026-09-11 → 2026-10-11：显示 30 天 / 0y 1m 0d（9 月 30 天，正确）；
切到工作日页显示 21 个工作日、9 个周末日 —— 手算核对：30 天 = 4 整周（20 个工作日）
余 2 天，从 10-09 周五起算，周五 +1、周六不算 = 21 ✓。
周末日期的星期标记按 `isWeekend` 标成橙色，显示正常。
