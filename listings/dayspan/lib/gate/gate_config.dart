...
class GateConfig {
  ...
  static const String bundleId = 'com.haymoor.dayspan3306';
  ...
  ///
  /// **已填**（2026-09-11 建）。Adjust 里 App 名 `DaySpan`，Android 平台已配
  /// `com.haymoor.dayspan3306`（store Google Play），reporting currency PHP、
  /// 勾了「用户群仅在欧洲经济区之外」、App Links 关闭 —— 逐项与 ColorStack 一致。
  ///
  /// 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op。
  static String get adjustAppToken => 'jmnqnzl3ury8';
  ...
  ///
  /// **已填**（取自 `DaySpan` 的 Events 页）。`DaySpan` 下已建齐与 ColorStack 相同的
  ...
  static String get adjustOpenBLandingToken => '3e00vd';
}
