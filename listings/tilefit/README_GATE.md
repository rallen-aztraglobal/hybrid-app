...
- `adjustAppToken`：**待填，现在是 `TODO_ADJUST_APP_TOKEN`**。需要在 Adjust 后台为本包
  ...
  `com.emberlane.tilefit8264`、reporting currency 与现有 app 对齐用 **PHP**，建后不可改），
...
`com.emberlane.tilefit8264` 注册一个 Android App，下载 `google-services.json` 放到
...
带着它出包，等于在 TileFit 的 APK 里明写另外几个上架包的包名，任何人解压 APK 就能看出
...
缺这个文件时的降级路径已在 hexacolorsort 与 calcpad 上实测过：日志出现
`Default FirebaseApp failed to initialize because no default options were found`
之后 App 照常进 A 面、不崩 —— 即推送 no-op、不影响网关与游戏。本包当前就跑在这条
