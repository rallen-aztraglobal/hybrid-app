...
> LinkFlow is a logic puzzle game. Pairs of coloured dots sit on a grid; the player
> drags from one dot to its matching dot to draw a path between them. Paths may not
> cross each other, and every square on the board must end up covered. The app ships
> with a built-in set of levels across four difficulty tiers. There is no login, no
> subscription and no user-generated content. Which level the player is on is stored
> on the device.
...
> - shared_preferences — stores which level the player is on, locally on the device.
>
...
`https://frolicking-capybara-46647a.netlify.app/`

本包**专用**的 Netlify 站点，不与其余五个新包共用。内容是 `store/privacy-policy.html`
改名 `index.html` 部署的，所以根路径直接就是政策页。

2026-09-10 实测（未登录）：HTTP 200、标题 `Privacy Policy — LinkFlow`、
正文里的联系邮箱正确、无残留占位符。
...
本批三个游戏包共用这一个地址（本包 + NonoPix / MineGrid）。
...
- [x] 隐私政策 URL 已上线且可匿名访问 —— `https://frolicking-capybara-46647a.netlify.app/`，实测 HTTP 200、标题 `Privacy Policy — LinkFlow`
