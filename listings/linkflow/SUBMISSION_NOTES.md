...
本包与 nonopix / minegrid / unitshift / tickpad / checklane 是同一批、同一套 SDK、
...
| 证书 `O=` | ✅ 已各取各的（本包 `O=Wrenfield`；六个包分别是 Oakmere / Wrenfield / Pinehollow / Mossgate / Larkspur / Thornbury）|
| 隐私政策 URL | ✅ **已各建各的** —— 本包 `https://frolicking-capybara-46647a.netlify.app/` |
| 支持邮箱 | ⚠️ **本批三个游戏包共用** `appaztra@outlook.com`（与 NonoPix / MineGrid 同一个）——
...
`android:label` 与商店名都定为 `LinkFlow`，没有用 `Flow` 或 `Connect`。
那两个词在 Play 上同名应用成百上千，搜索里排不上；而且这一玩法的头部产品叫
`Flow Free`，取名太靠近容易被判为蹭名。`LinkFlow` 与包名
`com.wrenfield.linkflow6142` 一致。若已被占，退到 `LinkFlow — Connect Puzzle`。
...
本包挂 **`bp`**（BingoPlus），Console 里打开方式配**外开**。六个新包按 2/2/2 分到 ap / bp / gp。
...
```bash
adb shell dumpsys package com.wrenfield.linkflow6142 | sed -n '/requested permissions/,/install permissions/p'
```
...
com.wrenfield.linkflow6142.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION        ← androidx.core 的私有权限，非用户可见
...
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `r1akj2bp7xts` / `fqjboe` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.wrenfield.linkflow6142` 一条 client |
| 支持邮箱 | ✅ `appaztra@outlook.com`（**注意与另两个游戏包共用**）|
| 隐私政策 URL | ✅ `https://frolicking-capybara-46647a.netlify.app/`（实测匿名 HTTP 200）|
| 品牌归属 | ✅ `bp`（BingoPlus），打开方式**外开**，`bSideChromeColor` = `0xFFFFFFFF` |
