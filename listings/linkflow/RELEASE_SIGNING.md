...
**keystore 已生成** ✅ —— `android/linkflow6142.jks`（PKCS12 / RSA 2048 / 有效期到 2054-01），
alias `linkflow6142`，证书主体 `CN=LinkFlow, OU=Mobile, O=Wrenfield, L=Unknown, ST=Unknown, C=US`。
`android/key.properties` 已按它填好。两者都被 `.gitignore` 挡住，**不进 git**。

> `O=Wrenfield` 是本包**专用**的组织名 —— 证书主体谁都能看
...
在 `listings/linkflow/android/` 目录下执行，把 `<你的口令>` 换成自己的强口令：

```bash
keytool -genkeypair -v \
  -keystore linkflow6142.jks \
  -alias linkflow6142 \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -storepass '<你的口令>' -keypass '<你的口令>' \
  -dname "CN=LinkFlow, OU=Mobile, O=<你们的公司名>, L=<城市>, C=<国家代码>"
```
...
在 `listings/linkflow/android/` 下新建 `key.properties`：

```properties
storePassword=<你的口令>
keyPassword=<你的口令>
keyAlias=linkflow6142
storeFile=linkflow6142.jks
```
...
```bash
git status --porcelain listings/linkflow | grep -E '\.jks|key\.properties' && echo '停下！' || echo 'ok'
```
...
```
V2 Signer: certificate DN: CN=LinkFlow, OU=Mobile, O=Wrenfield, L=Unknown, ST=Unknown, C=US
V2 Signer: certificate SHA-256 digest: 4fe186c00dcece5f7b194fad23b4f818313556d96a3a4ecd68f2c16fcbb253d3
```
