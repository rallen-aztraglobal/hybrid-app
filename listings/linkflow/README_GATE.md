...
本工程（Flutter，**仅 Android**，包名 `com.wrenfield.linkflow6142`）已接入上架包 AB 面网关。
...
```json
{ "platform": "android", "bundleId": "com.wrenfield.linkflow6142", "timezone": "Asia/Manila" }
```
...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.wrenfield.linkflow6142`，
配好 palCode 与品牌。
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.wrenfield.linkflow6142` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`r1akj2bp7xts`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `fqjboe`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFFFFFFFF`（浅色站）。**若改挂 gp 深色站，要改成 `0xFF1C1D27` 并重新打包** |

Adjust 后台已给本包**单独建了 App**（名 `LinkFlow`，Android 平台 `com.wrenfield.linkflow6142`，
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.wrenfield.linkflow6142` 注册
（`mobilesdk_app_id` = `1:609439342540:android:ed2185e7b1896fafa98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
该文件随 APK 分发、反编译即可得，非机密，进 git。

**已按规矩裁剪成只含本包一个 client。** 控制台下载的原始文件会把该项目下**所有**
Android App 都列进来（本次下载里有 6 条），理由见 `SECURITY_NOTES.md`。
...
adb shell am force-stop com.wrenfield.linkflow6142
...
给游戏本体写 widget 测试时，直接挂 `GameScreen` 而**不是** `LinkFlowApp` ——
