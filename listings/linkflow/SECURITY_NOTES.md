...
其他 App 的 `client` 条目 —— 带着它出包，等于在 LinkFlow 的 APK 里明写另外几个上架包的
包名，正是这批包用不相关的厂商前缀（`wrenfield.linkflow6142` / `wrenfield` / `pinehollow` /
...
# 只应出现 com.wrenfield.linkflow6142 一行
```
...
```bash
adb shell dumpsys package com.wrenfield.linkflow6142 \
...
（`linkflow_solved_*` / `linkflow_level_*`，共 8 个）。非 root 设备上其他应用读不到，
...
unzip -p build/app/outputs/flutter-apk/app-release.apk assets/flutter_assets/kernel_blob.bin 2>/dev/null \
  ...
  | grep -v wrenfield.linkflow6142 || echo 'ok：没有别的包名'
