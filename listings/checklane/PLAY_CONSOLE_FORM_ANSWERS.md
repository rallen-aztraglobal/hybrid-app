...
> CheckLane is a checklist app for routines the user repeats. The user creates
> lists, types items into them, and ticks items off. A reset button clears the
> ticks while keeping the items, so the same list can be run again; a list can also
> be duplicated as a template. There is a thirty-step undo. Everything runs on the
> device: no login, no subscription, no sharing and no content downloaded for the
> lists. The lists — including the text the user types — are stored on the device.
...
> - shared_preferences — stores the user's checklists, including the text they
>   type, locally on the device.
>
...
`https://sensational-peony-d09cf4.netlify.app/`

本包**专用**的 Netlify 站点，不与其余五个新包共用。内容是 `store/privacy-policy.html`
改名 `index.html` 部署的，所以根路径直接就是政策页。

2026-09-10 实测（未登录）：HTTP 200、标题 `Privacy Policy — CheckLane`、
正文里的联系邮箱正确、无残留占位符。
...
本批三个工具包共用这一个地址（本包 + UnitShift / TickPad）。
...
- [x] 隐私政策 URL 已上线且可匿名访问 —— `https://sensational-peony-d09cf4.netlify.app/`，实测 HTTP 200、标题 `Privacy Policy — CheckLane`
