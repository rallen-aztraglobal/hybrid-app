...
其他 App 的 `client` 条目 —— 带着它出包，等于在 CheckLane 的 APK 里明写另外几个上架包的
包名，正是这批包用不相关的厂商前缀（`thornbury.checklane2894` / `wrenfield` / `pinehollow` /
...
# 只应出现 com.thornbury.checklane2894 一行
```
...
```bash
adb shell dumpsys package com.thornbury.checklane2894 \
...
`lib/storage/checklist_store.dart` 写一个 JSON 字符串（`checklane_lists_v1`），
**里面含用户自己写的清单标题与条目文字**。

这是本包与其余几个的实质差别：存的是自由文本而不是几个整数。非 root 设备上其他应用
读不到，卸载即删，**不上报也不随任何请求外发**。
...
unzip -p build/app/outputs/flutter-apk/app-release.apk assets/flutter_assets/kernel_blob.bin 2>/dev/null \
  ...
  | grep -v thornbury.checklane2894 || echo 'ok：没有别的包名'
