...
**本包与其余几个不同：存的是用户自己写的清单内容。** 别照抄「只存整数」的说法。

`pubspec.yaml` 里有 `shared_preferences`，`lib/storage/checklist_store.dart` 写一个
JSON 字符串：

| key | 内容 |
| --- | --- |
| `checklane_lists_v1` | 清单数组：`id` / **`title`（用户输入）** / `items[]`（每条含 **`text`（用户输入）** 与 `done`） |

写入时机：每一次勾选、增删改之后立刻写盘。读这一侧全程兜底，绝不抛异常。

**`title` 与 `text` 都是自由文本，理论上用户可以往里写任何东西。** 但它们**只存在本机
...
存了用户写的清单内容却在商店页说「什么都不存」，是没必要给自己留的漏洞。
