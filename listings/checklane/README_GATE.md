# CheckLane — AB 面网关接入说明

本工程（Flutter，**仅 Android**，包名 `com.thornbury.checklane2894`）已接入上架包 AB 面网关。
...
```json
{ "platform": "android", "bundleId": "com.thornbury.checklane2894", "timezone": "Asia/Manila" }
```
...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.thornbury.checklane2894`，
配好 palCode 与品牌。

**本包挂 `gp`**（GameZone 深色站）。六个新包按 2/2/2 分到 ap / bp / gp。

**打开方式：外开（系统浏览器）** —— B 面不在 App 内渲染，因此 `bSideChromeColor`
（本包 `0xFF1C1D27`）目前用不上。它仍按品牌取值，是因为打开方式由服务端下发，
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.thornbury.checklane2894` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`m4r7coxl34sg`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `h759bt`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFF1C1D27`（挂 `gp` 深色站）。**若改挂 ap/bp 浅色站，要改成 `0xFFFFFFFF` 并重新打包** |

Adjust 后台已给本包**单独建了 App**（名 `CheckLane`，Android 平台 `com.thornbury.checklane2894`，
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.thornbury.checklane2894` 注册
（`mobilesdk_app_id` = `1:609439342540:android:e837ae2ef9096c2da98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
该文件随 APK 分发、反编译即可得，非机密，进 git。

**已按规矩裁剪成只含本包一个 client。** 控制台下载的原始文件会把该项目下**所有**
Android App 都列进来（本次下载里有 10 条），理由见 `SECURITY_NOTES.md`。
...
adb shell am force-stop com.thornbury.checklane2894
...
给游戏本体写 widget 测试时，直接挂 `GameScreen` 而**不是** `CheckLaneApp` ——
