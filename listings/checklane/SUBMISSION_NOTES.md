...
本包与 nonopix / linkflow / minegrid / unitshift / tickpad 是同一批、同一套 SDK、
...
| 证书 `O=` | ✅ 已各取各的（本包 `O=Thornbury`；六个包分别是 Oakmere / Wrenfield / Pinehollow / Mossgate / Larkspur / Thornbury）|
| 隐私政策 URL | ✅ **已各建各的** —— 本包 `https://sensational-peony-d09cf4.netlify.app/` |
| 支持邮箱 | ⚠️ **本批三个工具包共用** `toolsa069@gmail.com`（与 UnitShift / TickPad 同一个）——
...
`android:label` 与商店名都定为 `CheckLane`，没有用 `Checklist` 或 `To-do`。
那两个词在 Play 上同名应用成千上万，搜索里排不上；而且 `To-do` 会把预期带偏 ——
本包不是待办，没有提醒和截止日期，用那个词只会招来因为「没有提醒」而给的差评。
`CheckLane` 与包名 `com.thornbury.checklane2894` 一致。
若已被占，退到 `CheckLane — Reusable Checklists`。
...
```bash
adb shell dumpsys package com.thornbury.checklane2894 | sed -n '/requested permissions/,/install permissions/p'
```
...
com.thornbury.checklane2894.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION        ← androidx.core 的私有权限，非用户可见
...
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `m4r7coxl34sg` / `h759bt` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.thornbury.checklane2894` 一条 client |
| 支持邮箱 | ✅ `toolsa069@gmail.com`（**注意与另两个工具包共用**）|
| 隐私政策 URL | ✅ `https://sensational-peony-d09cf4.netlify.app/`（实测匿名 HTTP 200）|
