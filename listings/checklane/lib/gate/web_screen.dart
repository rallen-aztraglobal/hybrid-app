...
/// 这里同口径。本包挂 `gp`（GameZone 深色站），故 `bSideChromeColor` 为 #1C1D27；
/// 改挂 ap/bp 时要改成白色，否则边色在对面站前会很突兀。见 GateConfig。
///
/// **注意：本页只在「内开」时才会被走到。** 本包当前在 Console 里配的是**外开**
/// （B 面交给系统浏览器，App 本体停在 A 面），因此 `bSideChromeColor` 目前是死值。
/// 之所以仍要让它与品牌一致：打开方式是**服务端下发**的，运营在 Console 里一改就生效，
/// 客户端不需要重新打包 —— 到那时这个常量立刻就有用了。
...
class _WebScreenState extends State<WebScreen> {
  ...
  @override
  void dispose() {
    // 外开失败会退回 A 面（工具），把系统 UI 模式还原，别把 edge-to-edge 带进工具页。
