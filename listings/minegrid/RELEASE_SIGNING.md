...
**keystore 已生成** ✅ —— `android/minegrid5083.jks`（PKCS12 / RSA 2048 / 有效期到 2054-01），
alias `minegrid5083`，证书主体 `CN=MineGrid, OU=Mobile, O=Pinehollow, L=Unknown, ST=Unknown, C=US`。
`android/key.properties` 已按它填好。两者都被 `.gitignore` 挡住，**不进 git**。

> `O=Pinehollow` 是本包**专用**的组织名 —— 证书主体谁都能看
...
在 `listings/minegrid/android/` 目录下执行，把 `<你的口令>` 换成自己的强口令：

```bash
keytool -genkeypair -v \
  -keystore minegrid5083.jks \
  -alias minegrid5083 \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -storepass '<你的口令>' -keypass '<你的口令>' \
  -dname "CN=MineGrid, OU=Mobile, O=<你们的公司名>, L=<城市>, C=<国家代码>"
```
...
在 `listings/minegrid/android/` 下新建 `key.properties`：

```properties
storePassword=<你的口令>
keyPassword=<你的口令>
keyAlias=minegrid5083
storeFile=minegrid5083.jks
```
...
```bash
git status --porcelain listings/minegrid | grep -E '\.jks|key\.properties' && echo '停下！' || echo 'ok'
```
...
```
V2 Signer: certificate DN: CN=MineGrid, OU=Mobile, O=Pinehollow, L=Unknown, ST=Unknown, C=US
V2 Signer: certificate SHA-256 digest: 69428baf65275258c499b16f64f3cd20a8be2fc06583ed002f5590517c697745
```
