...
class GateConfig {
  ...
  static const String bundleId = 'com.pinehollow.minegrid5083';
  ...
  /// **已填**（2026-09-09 建）。Adjust 里 App 名 `MineGrid`，Android 平台已配
  /// `com.pinehollow.minegrid5083`（store google play），reporting currency PHP、
  /// 勾了「用户群仅在欧洲经济区之外」—— 逐项与 ColorStack 一致。
  ///
  /// 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op。
  static String get adjustAppToken => 'u57nz7eapfcw';
  ...
  ///
  /// 品牌是按「A 面主题配 B 面站点主题」分的：游戏本体是深色
  /// （`AppColors.background` = #101319），配深色站，A→B 切换时系统栏不会突然一亮，
