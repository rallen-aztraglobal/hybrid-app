...
其他 App 的 `client` 条目 —— 带着它出包，等于在 MineGrid 的 APK 里明写另外几个上架包的
包名，正是这批包用不相关的厂商前缀（`pinehollow.minegrid5083` / `wrenfield` / `pinehollow` /
...
# 只应出现 com.pinehollow.minegrid5083 一行
```
...
```bash
adb shell dumpsys package com.pinehollow.minegrid5083 \
...
`lib/storage/progress_store.dart` 用 SharedPreferences 存八个整数
（`minegrid_wins_*` / `minegrid_best_*`，四个难度档各两个）。非 root 设备上其他应用读不到，
...
unzip -p build/app/outputs/flutter-apk/app-release.apk assets/flutter_assets/kernel_blob.bin 2>/dev/null \
  ...
  | grep -v pinehollow.minegrid5083 || echo 'ok：没有别的包名'
