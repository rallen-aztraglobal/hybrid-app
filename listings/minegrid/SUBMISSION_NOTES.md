...
本包与 nonopix / linkflow / unitshift / tickpad / checklane 是同一批、同一套 SDK、
...
| 证书 `O=` | ✅ 已各取各的（本包 `O=Pinehollow`；六个包分别是 Oakmere / Wrenfield / Pinehollow / Mossgate / Larkspur / Thornbury）|
| 隐私政策 URL | ✅ **已各建各的** —— 本包 `https://benevolent-moxie-f5b881.netlify.app/` |
| 支持邮箱 | ⚠️ **本批三个游戏包共用** `appaztra@outlook.com`（与 NonoPix / LinkFlow 同一个）——
...
`android:label` 与商店名都定为 `MineGrid`，没有用 `Minesweeper`。
那个词在 Play 上同名应用成千上万，搜索里根本排不上，且过于通用的名字容易被判为
误导性命名。`MineGrid` 与包名 `com.pinehollow.minegrid5083` 一致。
若已被占，退到 `MineGrid — Logic Mines`。
...
本包已定挂 `gp`（GameZone 深色站），`bSideChromeColor` = `0xFF1C1D27`。
品牌是按「A 面主题配 B 面站点主题」分的：本包是深色，配深色站，A→B 切换时系统栏
不会突然一亮。**若改挂 ap/bp（浅色站），要把该常量改回 `0xFFFFFFFF` 并重新打包。**
...
```bash
adb shell dumpsys package com.pinehollow.minegrid5083 | sed -n '/requested permissions/,/install permissions/p'
```
...
com.pinehollow.minegrid5083.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION        ← androidx.core 的私有权限，非用户可见
...
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `u57nz7eapfcw` / `obtq6c` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.pinehollow.minegrid5083` 一条 client |
| 支持邮箱 | ✅ `appaztra@outlook.com`（**注意与另两个游戏包共用**）|
| 隐私政策 URL | ✅ `https://benevolent-moxie-f5b881.netlify.app/`（实测匿名 HTTP 200）|
| 品牌归属 | ✅ `gp`（GameZone），打开方式**外开**，`bSideChromeColor` = `0xFF1C1D27` |
