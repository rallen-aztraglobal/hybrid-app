...
| 类别 | 文件 | 说明 |
| --- | --- | --- |
| 签名密钥 | `android/reptimer7719.jks` | release 签名密钥，泄露等于别人能签出被 Android 认作本 App 正式更新的包 |
...
```bash
git status --porcelain -uall listings/reptimer \
...
# 应为 0；把本包包名 com.quillford.reptimer7719 代进去应 > 0
```

## google-services.json 的裁剪

放入前必须**只保留本包的 client**。Firebase 控制台下载的那份会把同一项目下
**所有** App 都列进来 —— 带着它出包，等于在 RepTimer 的 APK 里明写另外十几个上架包的
包名，正是各包使用互不相关的厂商命名空间（`quillford` 等）想避免的事。
...
# 只应出现 com.quillford.reptimer7719 一行
```
...
本包比另外两个多一个依赖 `wakelock_plus`（计时期间保持屏幕常亮）。
**它没有新增任何权限** —— 用的是 `FLAG_KEEP_SCREEN_ON`，不需要权限；
而 `WAKE_LOCK` 本来就被 FCM 带进来了。这一条是实测比对过的，不是推断。

装好后核对实际权限：

```bash
adb shell dumpsys package com.quillford.reptimer7719 \
...
**本包存的是用户自由输入的文字**（方案名），不是几个整数 ——
DATA_SAFETY / PRIVACY_POLICY 里都单独写明了，别照抄另外两个包「只存整数」的说法。
