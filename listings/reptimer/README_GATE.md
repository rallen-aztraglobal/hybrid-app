...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.quillford.reptimer7719`，
palCode 填 `1123689307775344641`、品牌选 `gp`。

**本包挂 `gp`**（GameZone 深色站），PAL_CODE `1123689307775344641`（取自渠道 `gpgzmkk044`）。
九个上架包按 3/3/3 分到 ap / bp / gp。

**打开方式：外开（系统浏览器）** —— B 面不在 App 内渲染，因此 `bSideChromeColor`
（本包 `0xFF1C1D27`）当前用不上。它仍与品牌吻合（深色主题配深色站），
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.quillford.reptimer7719` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`9sgzjiaxgfeo`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `av2feh`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFF1C1D27`（挂 `gp`）。当前外开，这个值用不上，见上 |

Adjust 后台已给本包**单独建了 App**（名 `RepTimer`，Android 平台 `com.quillford.reptimer7719`，
...
`RepTimer` 下已建齐与 ColorStack 相同的 7 个事件：`AddToCart` / `CompleteRegistration` /
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.quillford.reptimer7719` 注册
（`mobilesdk_app_id` = `1:609439342540:android:d0c6a81f8fd6327da98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
...
adb shell am force-stop com.quillford.reptimer7719
