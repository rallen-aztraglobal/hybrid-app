...
本包与 EdgeLoop / DaySpan 是同一批，与更早的 nonopix / linkflow / minegrid / unitshift /
...
| 项目 | 要求 | 本包 |
| --- | --- | --- |
| 包名 | 各取不相关的厂商命名空间 | `quillford` |
| 证书 `O=` | **每包另取** | 见 `RELEASE_SIGNING.md` |
| 隐私政策 URL | **每包另建站点** | ✅ `https://splendid-frangipane-a4c517.netlify.app/` |
| 支持邮箱 | ⚠️ **按游戏/工具分两组共用** | `toolsa069@gmail.com` |
| `google-services.json` | 必须裁剪成只留本包 client | 见 `SECURITY_NOTES.md` |
| Adjust App Token | 必须新建 App，不可复用 | ✅ `9sgzjiaxgfeo` |
...
本包的支持邮箱是 `toolsa069@gmail.com`，与 UnitShift / TickPad / CheckLane / DaySpan **共用一个**。
...
`android:label` 与商店名都定为 `RepTimer`。
...
| 代码与测试 | ✅ analyze 干净，29 项测试全过；release APK 52.9MB |
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `9sgzjiaxgfeo` / `av2feh` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.quillford.reptimer7719` 一条 client |
| 支持邮箱 | ✅ `toolsa069@gmail.com`（**注意与其余工具包共用**）|
| 隐私政策 URL | ✅ `https://splendid-frangipane-a4c517.netlify.app/`（实测匿名 HTTP 200）|
| 品牌归属 | ✅ `gp`（GameZone），打开方式**外开**，`bSideChromeColor` = `0xFF1C1D27`（主题吻合，未改常量）|
...
启动示例方案 Tabata（准备 10 + 8×做 20 + 7×休 10 = 240 秒）。
第 14 秒时显示：阶段 WORK、本段剩 0:16、Round 1/8、总剩 3:46 ——
手算核对：准备 10 秒已过，做进行了 4 秒，剩 16 ✓；总剩 240-14=226 秒 = 3:46 ✓。
准备 → 做的阶段切换正常触发。
