...
本包与 nonopix / linkflow / minegrid / unitshift / checklane 是同一批、同一套 SDK、
...
| 证书 `O=` | ✅ 已各取各的（本包 `O=Larkspur`；六个包分别是 Oakmere / Wrenfield / Pinehollow / Mossgate / Larkspur / Thornbury）|
| 隐私政策 URL | ✅ **已各建各的** —— 本包 `https://delicate-seahorse-a2b478.netlify.app/` |
| 支持邮箱 | ⚠️ **本批三个工具包共用** `toolsa069@gmail.com`（与 UnitShift / CheckLane 同一个）——
...
`android:label` 与商店名都定为 `TickPad`，没有用 `Tally Counter` 或 `Counter`。
那两个词在 Play 上同名应用极多，搜索里排不上，且过于通用的名字容易被判为
误导性命名。`TickPad` 与包名 `com.larkspur.tickpad7358` 一致。
若已被占，退到 `TickPad — Tally Counter`。
...
```bash
adb shell dumpsys package com.larkspur.tickpad7358 | sed -n '/requested permissions/,/install permissions/p'
```
...
com.larkspur.tickpad7358.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION        ← androidx.core 的私有权限，非用户可见
...
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `oi2l8k6c3k00` / `9it54r` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.larkspur.tickpad7358` 一条 client |
| 支持邮箱 | ✅ `toolsa069@gmail.com`（**注意与另两个工具包共用**）|
| 隐私政策 URL | ✅ `https://delicate-seahorse-a2b478.netlify.app/`（实测匿名 HTTP 200）|
