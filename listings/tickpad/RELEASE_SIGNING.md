...
**keystore 已生成** ✅ —— `android/tickpad7358.jks`（PKCS12 / RSA 2048 / 有效期到 2054-01），
alias `tickpad7358`，证书主体 `CN=TickPad, OU=Mobile, O=Larkspur, L=Unknown, ST=Unknown, C=US`。
`android/key.properties` 已按它填好。两者都被 `.gitignore` 挡住，**不进 git**。

> `O=Larkspur` 是本包**专用**的组织名 —— 证书主体谁都能看
...
在 `listings/tickpad/android/` 目录下执行，把 `<你的口令>` 换成自己的强口令：

```bash
keytool -genkeypair -v \
  -keystore tickpad7358.jks \
  -alias tickpad7358 \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -storepass '<你的口令>' -keypass '<你的口令>' \
  -dname "CN=TickPad, OU=Mobile, O=<你们的公司名>, L=<城市>, C=<国家代码>"
```
...
在 `listings/tickpad/android/` 下新建 `key.properties`：

```properties
storePassword=<你的口令>
keyPassword=<你的口令>
keyAlias=tickpad7358
storeFile=tickpad7358.jks
```
...
```bash
git status --porcelain listings/tickpad | grep -E '\.jks|key\.properties' && echo '停下！' || echo 'ok'
```
...
```
V2 Signer: certificate DN: CN=TickPad, OU=Mobile, O=Larkspur, L=Unknown, ST=Unknown, C=US
V2 Signer: certificate SHA-256 digest: 3b278bc0bc5d65aebda8b621fd6e9225b4de671e0e0c228494833e2d18ed8b8b
```
