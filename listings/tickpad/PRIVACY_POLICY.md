# TickPad — Privacy Policy

**Last updated: 2026-09-09**

TickPad ("the app") is published by Aztra Global. This policy explains what the app
collects, why, and what it does not collect. It covers the Android app distributed on
Google Play under the package name `com.larkspur.tickpad7358`.
...
The app saves your counters and your saved sessions, using Android's standard
app-preferences storage:

| Key | Value |
| --- | --- |
| `tickpad_counters_v1` | your counters: the **name you typed**, current value, step, target, colour |
| `tickpad_sessions_v1` | saved sessions: a timestamp plus each counter's name and value |
| `tickpad_volume_keys` | whether volume-key counting is switched on |

This includes **text you type yourself** (counter names). It stays on your device: it is
never uploaded, never attached to analytics events, and never sent to our server.
"Copy summary" puts text on your clipboard for you to paste — the app does not send it
anywhere. Uninstalling the app deletes everything.
...
- You can delete everything the app stores locally by uninstalling it, or via
  **Settings → Apps → TickPad → Storage → Clear data**.
