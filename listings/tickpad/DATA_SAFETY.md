...
**本包与其余几个不同：存的不只是数字，还有用户自己起的名字。** 别照抄「只存整数」的说法。

`pubspec.yaml` 里有 `shared_preferences`，两个存储类各写一个 JSON 字符串：

| key | 内容 |
| --- | --- |
| `tickpad_counters_v1` | 计数器数组：`id` / **`label`（用户输入的名字）** / `value` / `step` / `target` / `color` |
| `tickpad_sessions_v1` | 会话历史数组：时间戳 + 每条的名字与数值 |
| `tickpad_volume_keys` | 音量键计数开关（布尔） |

写入时机：每一次加减、增删改、存会话之后立刻写盘（计数器常常是「按完就锁屏」，
晚写一步就可能少记一个）。读这一侧全程兜底，绝不抛异常。

**`label` 是自由文本，理论上用户可以往里写任何东西。** 但它**只存在本机
SharedPreferences 里**：不随网关判定请求上报、不进 push token 注册体、
不作为归因事件参数，卸载即随应用数据一并删除。「复制汇总」是把文本放进系统剪贴板，
交给用户自己去粘 —— App 不主动发到任何地方。

Play 的 Data safety 只要求申报**离开设备**的数据，纯本地数据不在申报范围内，
故 **Data safety 表单里不需要为它勾任何东西**。但隐私政策里必须如实写出来
（已写，见 `PRIVACY_POLICY.md` 的 "Stored only on your device"）——
存了用户输入的文字却在商店页说「什么都不存」，是没必要给自己留的漏洞。
