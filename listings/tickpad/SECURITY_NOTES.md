...
其他 App 的 `client` 条目 —— 带着它出包，等于在 TickPad 的 APK 里明写另外几个上架包的
包名，正是这批包用不相关的厂商前缀（`larkspur.tickpad7358` / `wrenfield` / `pinehollow` /
...
# 只应出现 com.larkspur.tickpad7358 一行
```
...
```bash
adb shell dumpsys package com.larkspur.tickpad7358 \
...
`lib/storage/counter_store.dart` 与 `session_store.dart` 各写一个 JSON 字符串
（`tickpad_counters_v1` / `tickpad_sessions_v1`），**里面含用户自己起的计数器名字**。

这是本包与其余几个的实质差别：存的不只是数字，还有自由文本。非 root 设备上其他应用
读不到，卸载即删，**不上报也不随任何请求外发**。「复制汇总」只是放进系统剪贴板，
由用户自己决定粘到哪。
...
unzip -p build/app/outputs/flutter-apk/app-release.apk assets/flutter_assets/kernel_blob.bin 2>/dev/null \
  ...
  | grep -v larkspur.tickpad7358 || echo 'ok：没有别的包名'
