...
class GateConfig {
  ...
  static const String bundleId = 'com.larkspur.tickpad7358';
  ...
  /// **已填**（2026-09-09 建）。Adjust 里 App 名 `TickPad`，Android 平台已配
  /// `com.larkspur.tickpad7358`（store google play），reporting currency PHP、
  /// 勾了「用户群仅在欧洲经济区之外」—— 逐项与 ColorStack 一致。
  ///
  /// 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op。
  static String get adjustAppToken => 'oi2l8k6c3k00';
  ...
  /// `Color.WHITE`。
  ///
  /// **本包挂 `bp`（BingoPlus，浅色站）**，故取白色。
  ///
  /// 六个新包按 2/2/2 分到 ap / bp / gp。**本包 A 面本体是深色主题，B 面却挂浅色站** ——
  /// 这在外开模式下毫无影响（B 面根本不在 App 内渲染）；万一改内开，白色边框配深色 A 面
  /// 会有一次明显的亮度跳变，那时应重新考虑品牌归属而不是把这个值改回深色。
  ///
  ...
  ///
  /// **若本包改挂 gp（深色站），这里要改成 0xFF1C1D27 并重新打包。**
