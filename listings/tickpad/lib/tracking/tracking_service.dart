...
/// - 进入 B 面时补一个 AF **标准事件** `af_content_view`（标准事件名，出现在工具类 App 里也不违和）。
...
class TrackingService {
  ...
  Future<void> _initAppsFlyer() async {
    // devKey 与 appId 都需配置才启用：本包仅发 Android，iOS 的 App Store ID 未定、appId 为占位，
    // 故 iOS 侧 AF 保持 no-op；Android 的 appId=包名恒有效，devKey 已填即启用。
    if (!_configured(GateConfig.appsFlyerDevKey) ||
        !_configured(GateConfig.appsFlyerAppId)) {
      return;
    }
    try {
