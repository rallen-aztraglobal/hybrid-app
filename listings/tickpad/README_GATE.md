...
本工程（Flutter，**仅 Android**，包名 `com.larkspur.tickpad7358`）已接入上架包 AB 面网关。
...
```json
{ "platform": "android", "bundleId": "com.larkspur.tickpad7358", "timezone": "Asia/Manila" }
```
...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.larkspur.tickpad7358`，
配好 palCode 与品牌。

**本包挂 `bp`**（BingoPlus 浅色站）。六个新包按 2/2/2 分到 ap / bp / gp。
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.larkspur.tickpad7358` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`oi2l8k6c3k00`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `9it54r`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFF1C1D27`（挂 `gp` 深色站）。**若改挂 ap/bp 浅色站，要改成 `0xFFFFFFFF` 并重新打包** |

Adjust 后台已给本包**单独建了 App**（名 `TickPad`，Android 平台 `com.larkspur.tickpad7358`，
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.larkspur.tickpad7358` 注册
（`mobilesdk_app_id` = `1:609439342540:android:5c871b3d749f4949a98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
...
adb shell am force-stop com.larkspur.tickpad7358
...
给游戏本体写 widget 测试时，直接挂 `GameScreen` 而**不是** `TickPadApp` ——
