# UnitShift — AB 面网关接入说明

本工程（Flutter，**仅 Android**，包名 `com.mossgate.unitshift4610`）已接入上架包 AB 面网关。
...
```json
{ "platform": "android", "bundleId": "com.mossgate.unitshift4610", "timezone": "Asia/Manila" }
```
...
Console → 上架包，新建一条：platform=`android`、bundleId=`com.mossgate.unitshift4610`，
配好 palCode 与品牌。
...
| 字段 | 现状 |
| --- | --- |
| `bundleId` | ✅ `com.mossgate.unitshift4610` |
| `apiBases` | ✅ 与其余上架包同一个渠道中台基址，可再加候选抗封 |
| `appsFlyerDevKey` | ✅ 账号级 key，与其余包共用（AF 按账号发，不按 App 发） |
| `adjustAppToken` | ✅ **`qv7x4u8c7bi8`** —— 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op（不崩，只是不上报） |
| `adjustOpenBLandingToken` | ✅ `7pdbn6`（Adjust 后台 event `OpenBLanding`）—— 与上面成对生效，缺一个就整条 no-op |
| `bSideChromeColor` | ✅ `0xFFFFFFFF`（浅色站）。**若改挂 gp 深色站，要改成 `0xFF1C1D27` 并重新打包** |

Adjust 后台已给本包**单独建了 App**（名 `UnitShift`，Android 平台 `com.mossgate.unitshift4610`，
...
**`hybrid-listings-51660`**（project_number `609439342540`）下以包名 `com.mossgate.unitshift4610` 注册
（`mobilesdk_app_id` = `1:609439342540:android:6c245d8d62504c2ca98769`），**不加 SHA 证书指纹** —— 与 ColorStack 一致。
...
adb shell am force-stop com.mossgate.unitshift4610
...
给游戏本体写 widget 测试时，直接挂 `GameScreen` 而**不是** `UnitShiftApp` ——
