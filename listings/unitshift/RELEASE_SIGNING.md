...
**keystore 已生成** ✅ —— `android/unitshift4610.jks`（PKCS12 / RSA 2048 / 有效期到 2054-01），
alias `unitshift4610`，证书主体 `CN=UnitShift, OU=Mobile, O=Mossgate, L=Unknown, ST=Unknown, C=US`。
`android/key.properties` 已按它填好。两者都被 `.gitignore` 挡住，**不进 git**。

> `O=Mossgate` 是本包**专用**的组织名 —— 证书主体谁都能看
...
在 `listings/unitshift/android/` 目录下执行，把 `<你的口令>` 换成自己的强口令：

```bash
keytool -genkeypair -v \
  -keystore unitshift4610.jks \
  -alias unitshift4610 \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -storepass '<你的口令>' -keypass '<你的口令>' \
  -dname "CN=UnitShift, OU=Mobile, O=<你们的公司名>, L=<城市>, C=<国家代码>"
```
...
在 `listings/unitshift/android/` 下新建 `key.properties`：

```properties
storePassword=<你的口令>
keyPassword=<你的口令>
keyAlias=unitshift4610
storeFile=unitshift4610.jks
```
...
```bash
git status --porcelain listings/unitshift | grep -E '\.jks|key\.properties' && echo '停下！' || echo 'ok'
```
...
```
V2 Signer: certificate DN: CN=UnitShift, OU=Mobile, O=Mossgate, L=Unknown, ST=Unknown, C=US
V2 Signer: certificate SHA-256 digest: cb31307ddf628be2539b7e108ce74cbc3648bb935a0b49c64a9284d9356090da
```
