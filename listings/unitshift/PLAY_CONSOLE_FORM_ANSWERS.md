...
> UnitShift is an offline unit converter. The user picks a category (length, mass,
> temperature, area, volume, speed, time, data, pressure, energy), types a number
> on the app's own keypad, and every unit in that category updates at the same
> time. Tapping a row makes it the source unit. All arithmetic happens on the
> device. There is no login, no subscription and no user-generated content. Only
> the last category and unit are remembered; the numbers typed are not stored.
...
> - shared_preferences — remembers the last category and unit, locally on the device.
>
...
`https://thriving-banoffee-396732.netlify.app/`

本包**专用**的 Netlify 站点，不与其余五个新包共用。内容是 `store/privacy-policy.html`
改名 `index.html` 部署的，所以根路径直接就是政策页。

2026-09-10 实测（未登录）：HTTP 200、标题 `Privacy Policy — UnitShift`、
正文里的联系邮箱正确、无残留占位符。
...
本批三个工具包共用这一个地址（本包 + TickPad / CheckLane）。
...
- [x] 隐私政策 URL 已上线且可匿名访问 —— `https://thriving-banoffee-396732.netlify.app/`，实测 HTTP 200、标题 `Privacy Policy — UnitShift`
