...
其他 App 的 `client` 条目 —— 带着它出包，等于在 UnitShift 的 APK 里明写另外几个上架包的
包名，正是这批包用不相关的厂商前缀（`mossgate.unitshift4610` / `wrenfield` / `pinehollow` /
...
# 只应出现 com.mossgate.unitshift4610 一行
```
...
```bash
adb shell dumpsys package com.mossgate.unitshift4610 \
...
`lib/storage/prefs_store.dart` 用 SharedPreferences 存几个短字符串
（`unitshift_category` / `unitshift_unit_*`），全是本表内置的标识符。
**用户敲进去的数字从不落盘** —— 只在内存里，关掉即无。
非 root 设备上其他应用读不到，卸载即删，也不上报。
...
unzip -p build/app/outputs/flutter-apk/app-release.apk assets/flutter_assets/kernel_blob.bin 2>/dev/null \
  ...
  | grep -v mossgate.unitshift4610 || echo 'ok：没有别的包名'
