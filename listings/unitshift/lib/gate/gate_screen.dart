...
/// 启动闸：App 的真正入口。启动即请求服务端 AB 面判定，据结果决定进 A 面（工具本体）
/// 还是 B 面（服务端下发的 web）。判定完成前显示与工具同底色的加载页，避免白屏。
///
/// 安全默认：判定进行中、失败、或结果非 B，一律进 A 面 —— 即工具本体（[ConverterScreen]），
/// 工具侧代码对网关零感知。
...
class _GateScreenState extends State<GateScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    if (result.isBSide) {
      ...
      if (result.isExternal) {
        // 外开：唤起系统浏览器打开 B 面，App 本体展示 A 面（工具）——既送用户去 B 面，
        // 又让 App 看起来仍是干净工具。浏览器打不开时静默降级，仍停在 A 面。
  ...
  Widget build(BuildContext context) {
    // 判定完成切 A/B 面时淡入，不生硬跳变（对齐 decktallypro setRoot 的 0.25s cross dissolve）。
...
/// 判定期加载页。底色沿用工具的 AppColors.background，判定结束切过去时背景不跳变。
