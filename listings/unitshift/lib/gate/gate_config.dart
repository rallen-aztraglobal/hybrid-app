...
class GateConfig {
  ...
  static const String bundleId = 'com.mossgate.unitshift4610';
  ...
  /// **已填**（2026-09-09 建）。Adjust 里 App 名 `UnitShift`，Android 平台已配
  /// `com.mossgate.unitshift4610`（store google play），reporting currency PHP、
  /// 勾了「用户群仅在欧洲经济区之外」—— 逐项与 ColorStack 一致。
  ///
  /// 若改回 `TODO_` 前缀，`TrackingService` 会跳过 Adjust 初始化，全链路 no-op。
  static String get adjustAppToken => 'qv7x4u8c7bi8';
  ...
  /// `Color.WHITE`。
  ///
  /// **本包挂哪个品牌还没定**，先按浅色站（`ap`/`bp`）取白色。
  ///
  /// 工具本体是浅色（`AppColors.background` = #F4F5F7），B 面也是浅色站，
  /// 所以 A→B 切换时系统栏不会突然变色，观感是连续的。这块区域归 B 面所有，
  /// 就该跟着 B 面走。  ///
  ...
  ///
  /// **若定为 gp（深色站），这里要改成 0xFF1C1D27 并重新打包。**
