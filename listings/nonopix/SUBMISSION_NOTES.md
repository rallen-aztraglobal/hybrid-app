...
本包与 linkflow / minegrid / unitshift / tickpad / checklane 是同一批、同一套 SDK、
...
| 证书 `O=` | ✅ 已各取各的（本包 `O=Oakmere`；六个包分别是 Oakmere / Wrenfield / Pinehollow / Mossgate / Larkspur / Thornbury）|
| 隐私政策 URL | ✅ **已各建各的** —— 本包 `https://moonlit-kangaroo-0a8292.netlify.app/` |
| 支持邮箱 | ⚠️ **本批三个游戏包共用** `appaztra@outlook.com`（与 LinkFlow / MineGrid 同一个）——
...
`android:label` 与商店名都定为 `NonoPix`，没有用 `Nonogram` 或 `Picross`。
那两个词在 Play 上同名应用成百上千，搜索里排不上，而且 `Picross` 是任天堂的注册商标，
用作应用名有被判侵权的风险。`NonoPix` 与包名 `com.oakmere.nonopix3927` 一致。
若 `NonoPix` 已被占，退到 `NonoPix — Picture Logic`。
...
```bash
adb shell dumpsys package com.oakmere.nonopix3927 | sed -n '/requested permissions/,/install permissions/p'
```
...
com.oakmere.nonopix3927.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION        ← androidx.core 的私有权限，非用户可见
...
| `adjustAppToken` / `adjustOpenBLandingToken` | ✅ `fpny286clibk` / `ur46ap` |
| `android/app/google-services.json` | ✅ 已放入，已裁剪成只留 `com.oakmere.nonopix3927` 一条 client |
| 支持邮箱 | ✅ `appaztra@outlook.com`（**注意与另两个游戏包共用**）|
| 隐私政策 URL | ✅ `https://moonlit-kangaroo-0a8292.netlify.app/`（实测匿名 HTTP 200）|
