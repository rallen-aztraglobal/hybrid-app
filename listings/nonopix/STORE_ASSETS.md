...
| `store/screenshot-1.png` | 1200×2400 | 手机截图 —— 未解的 5×5 盘面 |
| `store/screenshot-2.png` | 1200×2400 | 手机截图 —— 解完的「Boat」 |
...
- **图标**：格子拼出的一颗心。数织的全部意义就是「一格格涂出一幅图」，
  画一幅现成的像素图比画一个空棋盘更说明问题。
- **特色图**：一块横向的白色棋盘，上面涂出两幅小图（心 + 星）。

两者都直接取 `lib/theme/app_colors.dart` 的色值，商店图和实际画面对得上。
...
**第二张一定要截「已解开」的画面。** 数织的卖点是最后看见那幅图 ——
商店里只放一个空盘面，没人看得出这游戏有什么意思。

> 截图时踩到过一个真问题：船帆原来用米白 `#FFF3E0`，画在纯白棋盘上跟没涂一样
> （空格是 `#EBE7E0`，米白正好夹在中间）。已改成暖沙 `#E9C88C`。
> **加新图案时用截图确认一遍颜色在白底上看得见** —— 这个跑测试测不出来。
...
```bash
mkdir -p /tmp/nonopix-site && cp store/privacy-policy.html /tmp/nonopix-site/index.html
# 然后把 /tmp/nonopix-site 整个目录拖到静态托管上
```
...
curl -s  <你的URL> | grep -o '<title>[^<]*'   # 应为 <title>Privacy Policy — NonoPix
```
...
| 项 | 状态 |
| --- | --- |
| 支持邮箱 | ✅ `appaztra@outlook.com` —— **与 LinkFlow / MineGrid 共用**（运营决定） |
| 隐私政策 URL | ✅ `https://moonlit-kangaroo-0a8292.netlify.app/` —— 本包专用 Netlify 站点 |
