...
| key | 值 |
| --- | --- |
| `nonopix_solved_small` | 5×5 解开过多少题 |
| `nonopix_solved_large` | 8×8 解开过多少题 |

写入时机：每解开一题 +1。读写失败一律当「没有记录」处理（存档坏了顶多丢计数，
不影响开局）。

这两个整数**只存在本机 SharedPreferences 里**：不随网关判定请求上报、不进 push token
