...
**keystore 已生成** ✅ —— `android/nonopix3927.jks`（PKCS12 / RSA 2048 / 有效期到 2054-01），
alias `nonopix3927`，证书主体 `CN=NonoPix, OU=Mobile, O=Oakmere, L=Unknown, ST=Unknown, C=US`。
`android/key.properties` 已按它填好。两者都被 `.gitignore` 挡住，**不进 git**。

> `O=Oakmere` 是本包**专用**的组织名 —— 证书主体谁都能看
...
在 `listings/nonopix/android/` 目录下执行，把 `<你的口令>` 换成自己的强口令：

```bash
keytool -genkeypair -v \
  -keystore nonopix3927.jks \
  -alias nonopix3927 \
  -keyalg RSA -keysize 2048 -validity 10000 \
  -storepass '<你的口令>' -keypass '<你的口令>' \
  -dname "CN=NonoPix, OU=Mobile, O=<你们的公司名>, L=<城市>, C=<国家代码>"
```
...
在 `listings/nonopix/android/` 下新建 `key.properties`：

```properties
storePassword=<你的口令>
keyPassword=<你的口令>
keyAlias=nonopix3927
storeFile=nonopix3927.jks
```
...
```bash
git status --porcelain listings/nonopix | grep -E '\.jks|key\.properties' && echo '停下！' || echo 'ok'
```
...
```
V2 Signer: certificate DN: CN=NonoPix, OU=Mobile, O=Oakmere, L=Unknown, ST=Unknown, C=US
V2 Signer: certificate SHA-256 digest: e4fa95f08b2dc77ddc6c0c922359766625b094afc501ba832167f6b4661f6732
```
