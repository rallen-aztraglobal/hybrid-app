---
name: adjust-sync
description: 给渠道包批量新增/同步 Adjust 归因（建 app + Android 平台 + 按渠道模式建 6 个或「BP 原始事件」14 个事件），再把 app token 与事件 token 回填到线上 Console。用于"新增了小渠道后补 Adjust""同步 Adjust""给某某渠道建 Adjust app""补 adjust token 到线上"等场景。走 Adjust 前端内部接口（无需付费 Automation），幂等可重跑。
---

# Adjust 归因批量同步

给「渠道中台」里的小渠道包在 Adjust 后台批量建 app（含 Android 平台 + 转化事件：默认 6 个；Console 打开「BP 原始事件」开关的 BP 渠道建 14 个），并把生成的 **app token + 事件 token** 回填到线上 Console 的 `adjustAppToken` / `adjustEvents` 字段。方案背景见 [docs/admin/08-adjust.md](../../../docs/admin/08-adjust.md)、[ADR-0013](../../../docs/adr/0013-adjust-attribution.md) 与 [ADR-0018](../../../docs/adr/0018-adjust-bp-raw-events.md)（BP 原始事件模式，08 §11）。

## 什么时候用

- 新增了一批小渠道（`channels/*.csv` / Console 加了渠道），要给它们补上 Adjust。
- 想把线上所有 **未绑定 Adjust** 的 AP/BP 渠道一次性同步齐。
- 某个 BP 渠道刚在 Console 打开了「BP 原始事件」开关，要把它的 Adjust app 从 6 个事件补齐到 14 个。
- 只想给某几个指定渠道建 Adjust app。

> 只有 **AP / BP** 需要 Adjust（GP 不接）。默认处理线上 `adjustAppToken` 为空、或已开「BP 原始事件」但事件不齐 14 个的 AP/BP 渠道（增量），其余自动跳过。
...
- **事件集按渠道模式选（名字写死，必须与 App 端一致）**，由 Console 渠道的 `adjustBpRawEvents` 决定（pull_channels.py 带出，写在 BATCH 第 3 位）：
  - 默认（legacy）6 个：`AddToCart` / `CompleteRegistration` / `Login` / `OldRegPurchase` / `Purchase` / `TPFirstDeposit`，与 APK `AdjustBootstrap.LOGICAL_TO_ADJUST_NAME` 对齐。
  - 「BP 原始事件」（bpRaw，仅 BP）14 个：`Action_BUFD` / `Action_BURD` / `Action_Deposit` / `Action_FDRD` / `Action_Registration` / `ad_app_opened` / `ad_deeplink_opened` / `ad_deposit` / `ad_game_open` / `ad_registration` / `ad_web_deposit` / `ad_web_login` / `ad_web_pageview` / `ad_web_reg`，与 APK `BpRawAdjustTracker` 对齐（08-adjust.md §11.2）。`Action_*` 5 个 App 不发，为 S2S 预留。
  - 改动事件集时 provision.js `EVENT_SETS`、pull_channels.py `BP_RAW_EVENTS` 与 App 三处一起改。
...
输出 `$SC/adjust_channels.json`：全部 AP+BP 渠道（含 `id/flavor/applicationId/palCode/appName/adjustAppToken/adjustBpRawEvents/adjustEvents`）+ 打印「需同步（增量）」清单：未绑定的，以及已开 BP 原始事件但事件不齐 14 个的。
- 只想处理指定渠道：手工把 `delta` 里筛成你要的那几个 `[flavor, applicationId, bpRaw]`。

### 2. 在 Adjust 批量建 app（浏览器内部接口，幂等）

把第 1 步的**增量** `[[flavor, applicationId, bpRaw], ...]` 数组，**分批（每批 ≤14）** 内联进 [scripts/provision.js](scripts/provision.js) 里 `const BATCH = [...]`，用 `evaluate_script` 逐批执行。脚本按第 3 位 `bpRaw` 选事件集；每批返回每个渠道的 `{flavor, bpRaw, appToken, events:{name:token}, steps}`。
...
- 全部跑完后合并去重、校验：每个渠道都要有 `appToken` + 事件齐全（legacy 6 个 / bpRaw 14 个）、app token 互不相同。见 provision.js 末尾的合并/校验片段。
...
回填只改后台数据；要让**已有渠道**产出带新 token 的 APK，需在 Console「打包中心」或 `POST /api/build/jobs` 触发构建，CLI 会把 `adjustAppToken/adjustEvents`（bpRaw 渠道还有 `bpRawEvents`/`deepLinkHost`）渲染进 `app/adjust-tokens.json`（见 08-adjust.md §3/§5/§11）。bpRaw 渠道打包前记得在品牌域名卡片填好「Adjust 短链 host」。
